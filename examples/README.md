...
| `doc/ref/domains/01-user.md` | `ref/domains/` | 도메인 경계 한 장 |
| `doc/design/user/login/api-contract.ts` | `patterns/api-contract/template.ts` | 실제 로그인 계약 (tsc 통과) |
| `doc/design/user/login/20260714-task-login.md` | `patterns/task-doc/task-template.md` | 채워진 자연어 설계 |
| `doc/decisions/0001-jwt-over-session.md` | `decisions/` | ADR 한 장 |
| `doc/summary/20260714-summary-user-login.md` | `summary/` | `/sync`가 낸 작업 요약 |
...
| `doc/analysis/20260716-spike-approval-routing.md` | `analysis/` | 가설·검증기준·관찰·판정 결론 문서 |
| `doc/decisions/0002-approval-line-rule-engine.md` | `decisions/` | 승격된 결정(ADR) |
| `doc/ref/architecture/01-approval-routing.md` | `ref/architecture/` | 승격된 확정 제약 |

## 흐름으로 읽기

```
[신규 기능 — 자산 모드]
/design   → 20260714-task-login.md + api-contract.ts 생성, 0001 ADR 기록
/builder  → (실제 소스 코드 — 이 예시엔 생략) + task History 갱신
/sync     → 20260714-summary-user-login.md 생성, 계약 최종 정합
/commit   → 코드+문서 한 커밋 (push는 사람)

[신규 프로젝트 앞단 — 학습 모드]
/spike    → spike/approval-routing/ (버릴 코드)로 가설 검증
          → analysis/{date}-spike-*.md 결론(자동 96%+수동 4% 판정)
          → 지식만 승격: ADR 0002 · ref/architecture/01-approval-routing.md
