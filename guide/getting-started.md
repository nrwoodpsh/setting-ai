...
혼자면 넘어가도 된다. 2인 이상이면 **설계 문서 충돌**을 막는 규칙이 필요하다 — `doc/ref/domains/`가 도메인 중복은 막지만, 같은 도메인을 동시에 건드릴 때의 소유권까지는 정하지 않는다.

| 규칙 | 내용 |
|:--|:--|
| **1 브랜치 = 1 설계문서 소유권** | 기능 브랜치(`feature/user-login`)를 연 사람이 그 도메인 설계문서(`doc/design/user/login/`)의 소유자. 남의 브랜치 설계문서를 고치지 않는다. |
| **같은 도메인 동시 작업** | phase 경로를 쪼갠다 — `user/login-auth/`와 `user/login-history/`처럼 폴더 자체를 분리해 물리 충돌을 없앤다. |
| **설계문서(계약) 충돌 시** | 두 브랜치의 계약(`api-contract`)이 겹치면 **선행 PR의 계약을 정본**으로 채택하고, 후행 PR 작성자가 자기 설계를 그 정본에 맞춘다. |
| **파일명에 담당자 식별** *(협업 시 옵션)* | 기본은 `{date}-task-{name}.md`. 협업으로 헷갈리면 이니셜을 붙인다 — `20260721-task-login-KJH.md`. 1인 작업이면 기본형 그대로. |

- `doc/summary/`는 파일명에 날짜+task명이 박혀 물리 충돌이 거의 없다. 겹치면 일반 Git merge로 해결.
- **PR에 코드 + 설계문서 + summary를 함께** 올린다. 리뷰어가 설계문서 충돌·중복을 교차 확인한 뒤 승인. (push·merge는 사람이 외부 툴로 — §4와 동일.)
- 서버측 드리프트 강제가 필요하면 §운영의 CI 게이트(`drift-gate.yml.example`, `automation/README.md`)를 켠다.
