...
- **드리프트 게이트(서버사이드)**: 로컬 `drift-hook`은 `git push --no-verify`로 우회되니, PR에서 한 번 더 막고 싶으면 Actions로. 코드 변경이 있는데 `doc/summary/` 갱신이 없으면 실패 — 로컬 훅(우회 가능) + 이 게이트(우회 불가)로 이중 방어.
  - **씨앗 제공**: `project-template/.github/workflows/drift-gate.yml.example`가 `/flow:setup`으로 프로젝트에 함께 복사된다. flow 기본 철학상 하드 게이트는 옵트인이라 `.example`로 비활성 배포된다 — 활성화는 확장자만 벗기면 된다:
    ```bash
    mv .github/workflows/drift-gate.yml.example .github/workflows/drift-gate.yml
    ```
    차단이 아니라 알림만 원하면 파일 안의 `exit 1`→`exit 0`, `::error::`→`::warning::`. (판정 로직·설명은 씨앗 파일 헤더에.)
