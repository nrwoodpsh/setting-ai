...
**도메인 네이밍 규칙**: `doc/ref/domains/`의 기존 도메인을 먼저 확인해 **재사용**한다. 신규면 영문 kebab-case(예: `user`, `order`)로 짓되, 기존과 유사한 도메인이 있으면 새로 만들지 말고 **사용자에게 확인**한다(중복 도메인 방지). 새 도메인을 만들면 `doc/ref/domains/{NN}-{domain}.md`(다음 순번)에 경계를 한 줄이라도 남긴다.
...
3. **task 문서 작성**: `{date}-task-{name}.md` (날짜 앞 — `YYYYMMDD`) — Requirements / UI·UX / Logic / File Map / Verification / History. 섹션 골격은 `doc-template`, 문장 가독성은 `plain-writing` 스킬(쉬운 말 규칙)을 따른다. **산출은 "수정 가능성 순"으로 제시** — 판단이 갈리는 결정(스키마 선택·타입 인터페이스·사용자에게 보이는 것)은 **택한 안 + 기각한 대안**을 함께 앞에 두고, 기계적 작업(배선·리팩터)은 뒤로 묻는다. 실행 순서가 아니라 **주의를 기울일 것 먼저**.
...
6. **용어집·도메인 갱신 제안**: 설계 중 혼동 가능한 **새 용어**나 **새 도메인 경계**가 나오면 `doc/ref/glossary/`·`doc/ref/domains/`에 갱신을 **제안**한다(사람 확인). 이 둘은 상시 참조 정본이라, 여기 반영 안 하면 다음 설계가 옛 용어·옛 경계로 샌다. (알파/POC 후 AI가 만든 초안을 이어 다듬는 지점 — 백지 작성이 아니다.)
