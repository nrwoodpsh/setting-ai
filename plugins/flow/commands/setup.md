...
5. **추천 도구·자동화 제안**:
   ...
   - *(CI 드리프트 게이트 — 옵트인)* 로컬 훅은 `--no-verify`로 우회되므로, **서버측 우회 불가 게이트**를 원하는 팀에는 `project-template/.github/workflows/drift-gate.yml.example`가 함께 복사됨을 안내한다(활성화는 `.example` 확장자 벗기기 — **자동 활성화하지 않는다**, 하드 게이트는 사람 결정). 상세: `automation/README.md`.
