...
- **계약 파일과 중복 작성 금지** — task에는 의도·배경·UX만, 타입·Endpoint·에러코드는 계약만. 계약이 답해야 할 4가지(검증 규칙·에러 status·페이징 기준·FE 상태)는 `doc/ref/patterns/api-contract/` 템플릿 헤더 참조 — 필드명만 나열하면 FE/BE가 다르게 해석한다.
