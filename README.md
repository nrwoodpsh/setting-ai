...
AI 활용 성숙도 5단계(L1 단위 프롬프트 → L2 AI 코딩 도구 → L3 표준화·고도화 → L4 에이전트 워크플로우 → L5 자율 개발 조직) 중, setting-ai는 **L3을 완성하고 L4를 "규율형"으로 채택**한다.

| 단계 | 정의 | setting-ai |
|:--|:--|:--:|
| **L3** 표준화·고도화 | 프리셋·프롬프트 체계로 품질 통일 | ✅ **완전** (3층 패턴·커맨드/스킬 골격 표준·doc 규약) |
| **L4** 에이전트 워크플로우 | 단계별 에이전트·도구 연동 자동화 | 🔶 **규율형** (아래 두 축) |
| **L5** 자율 개발 조직 | AI가 수행, 사람은 정의·검증 | ✗ 실행 자율성은 **의도적 배제** |

**L4를 두 축으로 나누면 우리 위치가 분명해진다:**

- **축 A — 도구 연동 자동화 ✅**: `contract-gate` 훅이 편집마다 `tsc` 자동 실행, git 훅이 커밋/푸시 시 드리프트 자동 감지, `tdd-verify` 테스트 실행, `explorer`·`verifier` 서브에이전트 자동 위임.
- **축 B — 단계 자율 수행 ✗ (일부러 뺌)**: `/design→/builder→/sync`를 **사람이 하나씩 호출**한다. AI가 스스로 다음 단계·커밋·머지로 넘어가지 않는다 — **사람이 상태전이자**다.

**왜 자율 주행 버튼을 안 다나** — "초록불 ≠ 정확함"([`workflow-principles.md`](guide/workflow-principles.md) §6). 게이트는 명백한 오류만 거르지 계약이 현실과 맞음을 증명하진 않는다. 그래서 **국면 사이의 사람 게이트가 정합성을 지킨다** — 자동 커밋·실패 시 자율 수정·단계 자동 체이닝을 금지한다. L5의 노동 분업(사람=정의·검증, AI=실행)은 task 단위로 이미 쓰되, **실행 자율성(커밋·push·머지)은 L4에 묶어둔다.** "L5의 속도가 아니라 L5의 규율"을 택한 것이 이 워크플로우의 정체성이다.

> **에이전트 워크플로우는 언제든 돈다.** L4의 발판(국면 분리·게이트·서브에이전트·도구 연동)은 항상 가동된다 — 다만 국면과 국면 사이에 사람 확인을 끼울 뿐이다. 자동화 체감을 원하면 **선택 오버레이 [`/flow:run`](plugins/flow/commands/run.md)** 이 `design→builder→sync`를 자동으로 잇는다 — 단 하드 게이트는 그대로: **design 청사진 컨펌·실패 시 정지·commit 직전 정지·push는 사람.** 즉 "자율 주행"이 아니라 "게이트를 지키는 자동 진행"이다. commit까지 자동화(완전 자동 L4)하려면 §7(자동 커밋 금지)을 버려야 하므로, 그건 정체성을 바꾸는 별도 결정으로 남겨둔다.

---

## 구성 요소

### 커맨드 (12) — 호출은 `/flow:<이름>` (예: `/flow:design`)
...
| 오케스트레이션 | `/run` | design 컨펌 후 **builder→sync를 자동으로 잇고 commit 직전 정지** (선택 오버레이 — 규율형 L4) | 각 국면 산출물 + 커밋 미리보기 |
...
/flow:run 로그인 API AC: 성공→토큰 / 실패→U001 / 200ms   # design→builder→sync 자동, commit 직전 정지 (선택)
...
| `contract-gate` | 계약 검증 게이트 (스택무관) |
| `drift-check` | 커밋 준비 시 드리프트 확인 |
| `tdd-verify` | 테스트 실행으로 환각 방지 |
| `code-audit` | 스택무관 감사 체크리스트 |
| `doc-template` | task 문서 섹션 골격 |
| `plain-writing` | 자연어 문서 쉬운 말 규칙 |
| `impact-analysis` | 기존 로직 수정 전 회귀 표면 분석 |
...
│   │   ├── doc-template/         #     task-*.md 표준 섹션 골격
│   │   ├── plain-writing/        #     자연어 문서 쉬운 말 규칙 (가독성 게이트)
│   │   └── impact-analysis/      #     기존 로직 수정 전 회귀 표면 분석
│   └── hooks/
...
| 새 기능 (자동 진행) | `/flow:run` — design→builder→sync 자동, commit 직전 정지 (선택) |
