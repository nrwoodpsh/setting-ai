...
> **상태: v0.3 — 설치·발견·네임스페이스(`/flow:*`) 실측 확인됨.** 라이프사이클(원형→개발→발행)·자동화(drift 훅·버전관리)·디자인 테마·문서 골격 표준화·에피스테믹 기법(사각지대·인터뷰·수정가능성순·이탈로그)까지 갖췄다. 남은 실측: 실제 한 바퀴(`/flow:setup`→개발→`/flow:sync`→`/flow:publish`)·아키텍처 원형 clone·훅 발화 확인.
...
자세히: 사상 [`guide/workflow-principles.md`](guide/workflow-principles.md) · 구조·데이터 흐름 [`guide/doc-structure.md`](guide/doc-structure.md) · 사용법 [`guide/getting-started.md`](guide/getting-started.md) · 작성 표준 [`guide/authoring-standard.md`](guide/authoring-standard.md) · 완결 예시 [`examples/`](examples/).

---

## 구성 요소

### 커맨드 (10) — 호출은 `/flow:<이름>` (예: `/flow:design`)
| 커맨드 | 역할 |
|:---|:---|
...
| `/design` | 신규 기능 설계 → `task-*.md` + 계약. 구현 코드 금지. **사각지대 패스**(repo 이력의 함정 발굴)·**인터뷰**(모호함 1문1답)·**수정가능성순 산출**(결정+대안 먼저). 중요한 결정은 ADR |
| `/troubleshoot` | 터진 버그의 재현·근본원인·수정안. 근본원인 확정 전 수정 금지 |
| `/builder` | 설계대로 구현. TDD·계약게이트·셀프검증. **이탈 로그**(계획 벗어난 지점 실시간 기록 → `/sync` 입력). 막히면 리포트 후 승인 대기 |
...
| `/theme` | 디자인 스펙(getdesign.md 또는 로컬 md)을 프론트에 적용 — 토큰 전면+컴포넌트 룩(구조 재설계 제외) |
...
**디자인 입히기(선택, 프론트가 있을 때)** — `/setup`으로 원형 만든 뒤:
```
# ① (권장·정확) getdesign.md 사이트에서 디자인 DESIGN.md 다운로드 → 프로젝트 doc/ref/theme/ 에 넣기
/flow:theme @doc/ref/theme/DESIGN-apple.md
# ② (편의) 온라인 자동: 목록에서 고르기 (웹변환이라 토큰 부정확할 수 있음 → 검토)
/flow:theme
```
→ **적용**: 토큰(색·타이포·간격·radius) + **각 컴포넌트 룩**(버튼·카드·인풋·네비). **유지**: 화면 배치·구조(어떤 게 어디에·몇 컬럼·페이지 IA). **스킨만 갈고 구조는 그대로** — 전체 재설계 아님.
> **전체 순서**: `설치 → /flow:setup(+원형) → /flow:theme(디자인) → /flow:design→builder→sync→commit → /flow:publish`
...
### ⚙️ 오너 · 워크플로우 수정 → 배포 *(setting-ai를 고칠 때)*

**로컬 개발:**
```
/plugin marketplace add /path/to/setting-ai     # 로컬 마켓 등록 (최초 1회)
# 커맨드/스킬 수정 → /reload-plugins 로 반영·테스트 (반복)
```
...
git push                            # ⚠️ 버전 안 올렸으면 pre-push 가드가 막음
                                    #    (설치측 캐시 키=버전 → 안 올리면 update 해도 반영 안 됨)
```
...
/reload-plugins
```
> **pre-push 가드 최초 설치**(setting-ai repo에서 1회): `cp automation/git-hooks/pre-push-version-guard .git/hooks/pre-push && chmod +x .git/hooks/pre-push`
> 버전은 **커밋마다가 아니라 릴리스(배포)할 때 한 번** 올린다.
```
