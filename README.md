...
> **상태: v0.2 — 설치·발견·네임스페이스(`/flow:*`) 실측 확인됨.** 라이프사이클(원형→개발→발행)·자동화(drift 훅·버전관리)·디자인 테마까지 갖췄다. 남은 실측: 실제 한 바퀴(`/flow:setup`→개발→`/flow:sync`→`/flow:publish`)·아키텍처 원형 clone·훅 발화 확인.
