...
`/flow:setup`이 순서대로 (추론은 자동, 결정은 질문): ① 골격 생성(`CLAUDE.md`·`config`·`doc/`·`.claude/settings.json`) → ② 스택 스캔(빌드·테스트·계약 게이트 명령 추론) → ③ 아키텍처 원형(빈 프로젝트면 clone, 원본 `.git` 제거·LICENSE 유지) → ④ 추천 도구(LSP·MCP) + **drift 훅** 설치 → ⑤ 결정 항목만 질문(가드레일·도구정책·도메인 경계).
